...
class MELDDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        text = torch.FloatTensor(numpy.array(self.roberta1[vid]))
        speaker = torch.FloatTensor(numpy.array(self.videoSpeakers[vid]))
        mask = torch.FloatTensor(numpy.array([1]*len(self.videoLabels[vid])))
        
        label = torch.LongTensor(numpy.array(self.videoLabels[vid]))
